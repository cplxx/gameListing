...
interface Props {
  gameList: Game[];
  selectedGenresName: string;
}

function GamesByGenresId({ gameList, selectedGenresName }: Props) {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-5">
        {gameList.map((item, index) => (
          <div
            key={index}
            className="bg-[#302f2f5e] p-3 rounded-lg pb-12 h-full hover:scale-110 transition-all ease-in-out duration-300 cursor-pointer"
          >
            <img
